...
'@
        # Replace the directory string AND strip out Windows Carriage Returns (`r)
        $installCommand = $installCommand.Replace("__REMOTE_DIR__", $RemoteDir).Replace("`r", "")
        
        Write-Host "Installing Python dependencies on the Pi ..."
        ssh -tt "$PiUser@$PiHost" $installCommand
        if ($LASTEXITCODE -ne 0) {
