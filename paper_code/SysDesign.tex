...
At concept level, the system gives the teacher an additional source of information about classroom engagement during technology-supported lessons. It does not replace teacher judgement, evaluate students automatically, or make definitive claims about learning outcomes. Instead, it collects EEG samples from participating students, converts them into an engagement indicator, and presents the result in a form that can support reflection on lesson pacing, topic difficulty, and moments where attention may decrease. This cautious interpretation is consistent with previous classroom EEG studies, where engagement monitoring is treated as supportive information rather than as a direct measure of learning \cite{Poulsen2017,Apicella2022}.


In the intended classroom scenario, each participating student uses one Unicorn EEG headset connected to one Raspberry Pi acquisition node. The full classroom setup is obtained by repeating this one-student acquisition station for \(N\) students, so each station remains independent while all stations publish their streams to the same local network. Each node reads the signal from its associated headset and publishes it on the local network. On the teacher workstation, the application receives the available streams, identifies them by student or device label, computes the engagement value, and displays the information through a Streamlit interface. This creates a complete path from physiological acquisition to classroom-facing feedback, while keeping the hardware, network, processing, and visualization responsibilities clearly separated.


One important design principle is to keep the student-side node simple. Its role is limited to acquisition and transmission, without any pedagogical decision-making. Interpretation, visualization, lesson management, and reporting are handled on the teacher workstation, where the logic is easier to inspect, update, and test. As a result, changes to the engagement metric or to the interface do not require the acquisition nodes to be redesigned.


Teacher-centered use also guided the design of the interface. Rather than building the system around student ranking, permanent profiles, or competitive comparisons, the prototype focuses on lesson activity and class-level reflection. This is more appropriate for the educational purpose of the work: the teacher can notice changes in engagement and relate them to instructional decisions, without reducing students to a single numerical score.


A live signal monitor alone would not be sufficient for this use case. For that reason, the prototype includes the surrounding teaching workflow as well. Lesson materials can be uploaded, previewed, presented, and later reviewed through reports. EEG-derived engagement becomes more meaningful when it is connected to lesson context; for example, a decrease during a particular slide is more useful when the teacher can relate it to the topic, explanation, or activity taking place at that moment.


Throughout the system, the engagement value is treated as an indicator rather than as an absolute measurement. Classroom EEG can be influenced by movement, electrode contact, individual variation, and environmental noise, which are common limitations of portable EEG technologies in educational research \cite{Xu2018,BustosLopez2022}. Consequently, the interface is intended to support observation and reflection, not ranking or disciplinary decisions. The final interpretation remains pedagogical and human, with the system serving as an aid rather than an authority.
